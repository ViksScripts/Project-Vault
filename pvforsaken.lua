...
local function GetWriteFile()
    local wf = writefile
    if not wf then
        wf = syn and syn.writefile
    end
    if not wf then
        wf = fluxus and fluxus.writefile
    end
    if not wf then
        wf = secure_load and secure_load.writefile
    end
    return wf
end
...
local function GetReadFile()
    local rf = readfile
    if not rf then
        rf = syn and syn.readfile
    end
    if not rf then
        rf = fluxus and fluxus.readfile
    end
    if not rf then
        rf = secure_load and secure_load.readfile
    end
    return rf
end
...
local function GetIsFile()
    local isf = isfile
    if not isf then
        isf = syn and syn.isfile
    end
    if not isf then
        isf = fluxus and fluxus.isfile
    end
    if not isf then
        isf = secure_load and secure_load.isfile
    end
    return isf
end

local function GetIsFolder()
    local isf = isfolder
    if not isf then
        isf = syn and syn.isfolder
    end
    if not isf then
        isf = fluxus and fluxus.isfolder
    end
    if not isf then
        isf = secure_load and secure_load.isfolder
    end
    return isf
end
...
local function GetMakeFolder()
    local mf = makefolder
    if not mf then
        mf = syn and syn.makefolder
    end
    if not mf then
        mf = fluxus and fluxus.makefolder
    end
    if not mf then
        mf = secure_load and secure_load.makefolder
    end
    return mf
end

local writefile = GetWriteFile()
local readfile = GetReadFile()
local isfile = GetIsFile()
local isfolder = GetIsFolder()
local makefolder = GetMakeFolder()

local HAS_FILE_SYSTEM = not not (writefile and readfile and isfile and isfolder and makefolder)

local function GetHookMetamethod()
    local hm = hookmetamethod
    if not hm then
        hm = syn and syn.hook_metamethod
    end
    if not hm then
        hm = fluxus and fluxus.hook_metamethod
    end
    if not hm then
        hm = secure_load and secure_load.hook_metamethod
    end
    return hm
end

local function GetGetGC()
    local gc = getgc
    if not gc then
        gc = syn and syn.get_gc
    end
    if not gc then
        gc = fluxus and fluxus.get_gc
    end
    if not gc then
        gc = secure_load and secure_load.get_gc
    end
    return gc
end

local function GetNewCClosure()
    local nc = newcclosure
    if not nc then
        nc = syn and syn.new_cclosure
    end
    if not nc then
        nc = fluxus and fluxus.new_cclosure
    end
    if not nc then
        nc = secure_load and secure_load.new_cclosure
    end
    return nc
end

local function GetCheckCaller()
    local cc = checkcaller
    if not cc then
        cc = syn and syn.check_caller
    end
    if not cc then
        cc = fluxus and fluxus.check_caller
    end
    if not cc then
        cc = secure_load and secure_load.check_caller
    end
    return cc
end

local hookmetamethod = GetHookMetamethod()
local getgc = GetGetGC()
local newcclosure = GetNewCClosure()
local checkcaller = GetCheckCaller()

local HAS_HOOKMETAMETHOD = not not hookmetamethod
local HAS_GETGC = not not getgc

local CONFIG_FOLDER = "ProjectVault"
local CONFIG_FILE = "config.json"
local AUTO_LOAD_FILE = "autoload.json"
...
local function GetAutoLoadPath()
    return CONFIG_FOLDER .. "/" .. AUTO_LOAD_FILE
end

local function EnsureConfigFolder()
    if HAS_FILE_SYSTEM then
...
local function SaveConfig(ConfigData)
    if not HAS_FILE_SYSTEM then
        StarterGui:SetCore("SendNotification", {
            Title = "Error",
            Text = "Your executor does not support file system",
            Duration = 3
        })
        return false
    end
...
local function SetAutoLoad(Enabled)
    ...
    if Success then
        ...
        warn("Failed to set auto-load: " .. tostring(Err))
        return false
    end
end
...
local function SetupHookMetamethod()
    if not HAS_HOOKMETAMETHOD or not HAS_GETGC then
        warn("hookmetamethod or getgc not supported, some features may be unavailable")
        return false
    end
    ...
    local nc = newcclosure or DummyFunction
    local cc = checkcaller or DummyFunction
    ...
    local function CreateHook()
        ...
        local Success, Result = pcall(function()
            return hookmetamethod(game, "__namecall", nc(function(self, ...)
                if not cc() and State.IsUnderground and getnamecallmethod() == "FireServer" and self == UnreliableRemoteEvent and State.Invincible then
                    local Args = {...}
                    if Args[1] == 1 then
        ...
        if Success and Result then
            ...
            warn("Failed to setup hookmetamethod: " .. tostring(Result))
            return nil
        end
    end
...
local function GoUnder(Value)
    ...
    if Value and not State.IsUnderground then
        ...
        if HAS_HOOKMETAMETHOD and not State.HookMetamethodActive then
            SetupHookMetamethod()
        end
...
local UseActorAbility = GetNetworkRemote()
...
local function GetCurrentConfig()
    return {
        AutoGeneratorPuzzle = State.AutoGeneratorPuzzle,
        GeneratorCooldown = State.GeneratorCooldown,
        SpeedUpCooldown = State.SpeedUpCooldown,
        AutoPickup = State.AutoPickup,
        AutoEscape = State.AutoEscape,
        EscapeCooldown = State.EscapeCooldown,
        AutoDisarm = State.AutoDisarm,
        Invincible = State.Invincible,
        DisableKillerWalls = State.DisableKillerWalls,
        DisableToxicTrails = State.DisableToxicTrails,
        DisableFootprints = State.DisableFootprints,
        SmallerSpikeCollisions = State.SmallerSpikeCollisions,
        EnableJumping = State.EnableJumping,
        StaminaPreset = State.StaminaPreset,
        AntiSlowness = State.AntiSlowness,
        AnimationChanger = State.AnimationChanger,
        AnimationChangerValue = State.AnimationChangerValue,
        ChangeInLobby = State.ChangeInLobby,
        NoliControl = State.NoliControl,
        ControllableDash = State.ControllableDash,
        AutoBlock = State.AutoBlock,
        ESP = State.ESP,
        ShowText = State.ShowText,
        KillersESP = State.KillersESP,
        KillersColor = State.KillersColor,
        SurvivorsESP = State.SurvivorsESP,
        SurvivorsColor = State.SurvivorsColor,
        GeneratorsESP = State.GeneratorsESP,
        GeneratorsColor = State.GeneratorsColor,
        ItemsESP = State.ItemsESP,
        ItemsColor = State.ItemsColor,
        GeneratorsCheck = State.GeneratorsCheck,
        DisableNoliNPC = State.DisableNoliNPC,
        Disable007n7NPC = State.Disable007n7NPC,
        ExtendedFOV = State.ExtendedFOV,
        ExtendedZoom = State.ExtendedZoom,
        ShowChat = State.ShowChat,
        ShowPrivacy = State.ShowPrivacy,
        HideInjury = State.HideInjury,
        DisableBlindness = State.DisableBlindness,
        DeleteRagdolls = State.DeleteRagdolls,
    }
end

local function ApplyConfig(ConfigData)
    if not ConfigData then return end
    
    if ConfigData.AutoGeneratorPuzzle ~= nil then State.AutoGeneratorPuzzle = ConfigData.AutoGeneratorPuzzle end
    if ConfigData.GeneratorCooldown ~= nil then State.GeneratorCooldown = ConfigData.GeneratorCooldown end
    if ConfigData.SpeedUpCooldown ~= nil then State.SpeedUpCooldown = ConfigData.SpeedUpCooldown end
    if ConfigData.AutoPickup ~= nil then State.AutoPickup = ConfigData.AutoPickup end
    if ConfigData.AutoEscape ~= nil then State.AutoEscape = ConfigData.AutoEscape end
    if ConfigData.EscapeCooldown ~= nil then State.EscapeCooldown = ConfigData.EscapeCooldown end
    if ConfigData.AutoDisarm ~= nil then State.AutoDisarm = ConfigData.AutoDisarm end
    if ConfigData.Invincible ~= nil then State.Invincible = ConfigData.Invincible end
    if ConfigData.DisableKillerWalls ~= nil then State.DisableKillerWalls = ConfigData.DisableKillerWalls end
    if ConfigData.DisableToxicTrails ~= nil then State.DisableToxicTrails = ConfigData.DisableToxicTrails end
    if ConfigData.DisableFootprints ~= nil then State.DisableFootprints = ConfigData.DisableFootprints end
    if ConfigData.SmallerSpikeCollisions ~= nil then State.SmallerSpikeCollisions = ConfigData.SmallerSpikeCollisions end
    if ConfigData.EnableJumping ~= nil then State.EnableJumping = ConfigData.EnableJumping end
    if ConfigData.StaminaPreset ~= nil then State.StaminaPreset = ConfigData.StaminaPreset end
    if ConfigData.AntiSlowness ~= nil then State.AntiSlowness = ConfigData.AntiSlowness end
    if ConfigData.AnimationChanger ~= nil then State.AnimationChanger = ConfigData.AnimationChanger end
    if ConfigData.AnimationChangerValue ~= nil then State.AnimationChangerValue = ConfigData.AnimationChangerValue end
    if ConfigData.ChangeInLobby ~= nil then State.ChangeInLobby = ConfigData.ChangeInLobby end
    if ConfigData.NoliControl ~= nil then State.NoliControl = ConfigData.NoliControl end
    if ConfigData.ControllableDash ~= nil then State.ControllableDash = ConfigData.ControllableDash end
    if ConfigData.AutoBlock ~= nil then State.AutoBlock = ConfigData.AutoBlock end
    if ConfigData.ESP ~= nil then State.ESP = ConfigData.ESP end
    if ConfigData.ShowText ~= nil then State.ShowText = ConfigData.ShowText end
    if ConfigData.KillersESP ~= nil then State.KillersESP = ConfigData.KillersESP end
    if ConfigData.KillersColor ~= nil then State.KillersColor = ConfigData.KillersColor end
    if ConfigData.SurvivorsESP ~= nil then State.SurvivorsESP = ConfigData.SurvivorsESP end
    if ConfigData.SurvivorsColor ~= nil then State.SurvivorsColor = ConfigData.SurvivorsColor end
    if ConfigData.GeneratorsESP ~= nil then State.GeneratorsESP = ConfigData.GeneratorsESP end
    if ConfigData.GeneratorsColor ~= nil then State.GeneratorsColor = ConfigData.GeneratorsColor end
    if ConfigData.ItemsESP ~= nil then State.ItemsESP = ConfigData.ItemsESP end
    if ConfigData.ItemsColor ~= nil then State.ItemsColor = ConfigData.ItemsColor end
    if ConfigData.GeneratorsCheck ~= nil then State.GeneratorsCheck = ConfigData.GeneratorsCheck end
    if ConfigData.DisableNoliNPC ~= nil then State.DisableNoliNPC = ConfigData.DisableNoliNPC end
    if ConfigData.Disable007n7NPC ~= nil then State.Disable007n7NPC = ConfigData.Disable007n7NPC end
    if ConfigData.ExtendedFOV ~= nil then State.ExtendedFOV = ConfigData.ExtendedFOV end
    if ConfigData.ExtendedZoom ~= nil then State.ExtendedZoom = ConfigData.ExtendedZoom end
    if ConfigData.ShowChat ~= nil then State.ShowChat = ConfigData.ShowChat end
    if ConfigData.ShowPrivacy ~= nil then State.ShowPrivacy = ConfigData.ShowPrivacy end
    if ConfigData.HideInjury ~= nil then State.HideInjury = ConfigData.HideInjury end
    if ConfigData.DisableBlindness ~= nil then State.DisableBlindness = ConfigData.DisableBlindness end
    if ConfigData.DeleteRagdolls ~= nil then State.DeleteRagdolls = ConfigData.DeleteRagdolls end
    
    if ConfigData.StaminaPreset then
        UpdateStaminaPreset(ConfigData.StaminaPreset)
    end
    if ConfigData.ExtendedFOV then
        ...
        if Settings then
            ...
            if FOV then FOV.Value = ConfigData.ExtendedFOV end
        end
    end
    if ConfigData.ExtendedZoom then
        LocalPlayer.CameraMaxZoomDistance = game:GetService("StarterPlayer").CameraMaxZoomDistance + (ConfigData.ExtendedZoom * 0.25)
    end
    if ConfigData.ShowChat then
        local ChatService = game:GetService("TextChatService")
        local Config = ChatService:FindFirstChildOfClass("ChatWindowConfiguration")
        if Config then Config.Enabled = ConfigData.ShowChat end
    end
    if ConfigData.EnableJumping and LocalHumanoid then
        LocalHumanoid.JumpPower = ConfigData.EnableJumping and 50 or 0
    end
    if ConfigData.AnimationChanger and LocalHumanoid then
        UpdateAnim(LocalHumanoid)
    end
    if ConfigData.NoliControl then
        SetupNoliControl(ConfigData.NoliControl)
    end
    if ConfigData.Invincible then
        local Character = LocalPlayer.Character
...
local function InitializeModules()
    ...
    if not State.MainModule and HAS_GETGC then
...
local function UpdateCharacter(Character)
    ...
    if State.Invincible and HAS_HOOKMETAMETHOD then
        SetupHookMetamethod()
    end
end
...
ConfigGroup:CreateButton({
    ...
    Callback = function()
        local ConfigData = GetCurrentConfig()
...
ConfigGroup:CreateToggle({
    Text = "Auto-Load On Startup",
    Default = CheckAutoLoad(),
    Callback = function(Value)
        SetAutoLoad(Value)
    end
})
...
Auto Generators: %s
    ...
    HAS_HOOKMETAMETHOD and "Available" or "Unavailable",
    HAS_GETGC and "Available" or "Unavailable",
    HAS_FILE_SYSTEM and "Available" or "Unavailable",
    State.IsRequireSupported and "Available" or "Unavailable",
...
SystemGroup:CreateButton({
    ...
    Callback = function()
        if HAS_HOOKMETAMETHOD then
            ...
            StarterGui:SetCore("SendNotification", {
                Title = "Error",
                Text = "hookmetamethod not supported",
                Duration = 3
            })
        end
    end
...
print("hookmetamethod: " .. (HAS_HOOKMETAMETHOD and "Available" or "Unavailable"))
print("getgc: " .. (HAS_GETGC and "Available" or "Unavailable"))
print("File System: " .. (HAS_FILE_SYSTEM and "Available" or "Unavailable"))
print("Auto-Load: " .. (CheckAutoLoad() and "Enabled" or "Disabled"))